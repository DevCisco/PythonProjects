...
- Extracts fields (frame.number, frame.time_epoch, ip.src, ip.dst, udp.srcport, udp.dstport, data.data)
  from the filtered pcap and tries to decode data.data as UTF-8.
- Produces frames_index.csv and report.csv (filename, size, sha256, mtime_utc, source_frame)
"""
...
def sha256_of_file(path): # compute sha256 of a file
    h = hashlib.sha256()
...
def iso_utc_from_epoch(epoch_str): # convert epoch string to ISO 8601 UTC
    try:
...
def hex_to_bytes(h): # convert hex string (possibly with non-hex chars) to bytes
    if not h:
        return b''
...
def run_tshark_extract(tshark, pcap_in, tshark_filter, out_tshark_tsv): # request several possible data fields (different Wireshark versions / dissectors)
    fields = [
        "frame.number",
        "frame.time_epoch",
        "ip.src",
        ...
        "udp.srcport",
        "udp.dstport",
        # request multiple possible payload fields; tshark will output empty values if a field is not present
        "data.data"
    ]
...
def write_report(report_path, entries): # write report CSV
    # entries: list of tuples (filename, size_bytes, sha256, mtime_iso, source_frame)
    with open(report_path, "w", newline="", encoding="utf-8") as mf:
        w = csv.writer(mf)
...
def main():
    parser = argparse.ArgumentParser(description="Extract JSON payloads from pcap (forensic), compute hashes and report.")
    ...
    # 4) parse TSV and save hex payloads, build frames_index rows

    frames_index_rows = []  # (frame, epoch_ts, ip_src, ip_dst, udp_sport, udp_dport, json_filename)
    report_entries = []  # (filename, size_bytes, sha256, mtime_iso, source_frame)
    ...
    with open(tsv_path, "r", encoding="utf-8", errors="ignore") as tsvf:
        ...
        # heuristics to find useful column frames_indexes
    ...
    with open(tsv_path, "r", encoding="utf-8", errors="ignore") as tsvf:
        ...
        for i, parts in enumerate(reader, start=1):
            ...
            frames_index_rows.append((frame_no, epoch_ts, ip_src, ip_dst, udp_sport, udp_dport))
            payload_rows.append((hex_payload, payload_sha))

    # write frames_index.csv
    frames_index_csv = os.path.join(outdir, "frames_index.csv")
    with open(frames_index_csv, "w", newline="", encoding="utf-8") as idxf:
        w = csv.writer(idxf)
        w.writerow(["frame","time_epoch","ip_src","ip_dst","udp_src","udp_dst","payload_hex"])
        for r in frames_index_rows:
            w.writerow(r)

# finire di scrivere payload.csv
    raw_payload_csv = os.path.join(outdir, "payload.csv")
    with open(raw_payload_csv, "w", newline="", encoding="utf-8") as paycsv:
        payloadwrite = csv.writer(paycsv)
        payloadwrite.writerow(["frame","time_epoch","ip_src","ip_dst","udp_src","udp_dst", "payload_hex","payload_sha",])
        for rows in payload_rows:
            payloadwrite.writerow(rows)

    # 5) Build report entries: original pcap, filtered pcap, and each json file
    # original pcap
    mtime_orig = datetime.datetime.utcfromtimestamp(os.path.getmtime(pcap)).replace(microsecond=0).isoformat() + "Z"
    report_entries.append((os.path.basename(pcap), size_orig, sha_orig, mtime_orig, "original_capture"))

    # filtered pcap
    mtime_f = datetime.datetime.utcfromtimestamp(os.path.getmtime(filtered_pcap)).replace(microsecond=0).isoformat() + "Z"
    report_entries.append((os.path.basename(filtered_pcap), size_filtered, sha_filtered, mtime_f, "filtered_capture"))

    #sha frames_index_csv
    sha_frames_index = sha256_of_file(frames_index_csv)
    size_frames_index = os.path.getsize(frames_index_csv)
    
    #sha raw_payload_csv
    sha_pay = sha256_of_file(raw_payload_csv)
    size_pay = os.path.getsize(raw_payload_csv)

    mtime_paycsv = datetime.datetime.utcfromtimestamp(os.path.getmtime(raw_payload_csv)).replace(microsecond=0).isoformat() + "Z"
    report_entries.append((os.path.basename(raw_payload_csv), size_pay, sha_pay, mtime_paycsv, "payloads_csv"))

    mtime_frames_indexcsv = datetime.datetime.utcfromtimestamp(os.path.getmtime(frames_index_csv)).replace(microsecond=0).isoformat() + "Z"
    report_entries.append((os.path.basename(frames_index_csv), size_frames_index, sha_frames_index, mtime_frames_indexcsv, "frames_index_csv"))

    try:
        script_path = os.path.abspath(__file__)
        if os.path.exists(script_path):
            sha_script = sha256_of_file(script_path)
            size_script = os.path.getsize(script_path)
            mtime_script = datetime.datetime.utcfromtimestamp(os.path.getmtime(script_path)).replace(microsecond=0).isoformat() + "Z"
            report_entries.append((os.path.basename(script_path), size_script, sha_script, mtime_script, "tool_script"))
    except Exception:
       # non-fatal: continue without adding script entry
        pass

    # write report.csv
    report_csv = os.path.join(outdir, "report.csv")
    write_report(report_csv, report_entries)
    ...
    print("Hex payload saved to:", raw_payload_csv)
    print("Report:", report_csv)
    print("Frames index:", frames_index_csv)
    print("If desired, mount a VeraCrypt volume and copy the folder", outdir, "inside it. Then compute SHA256 of the container and store alongside report for chain of custody.")
    print("Done.")

if __name__ == "__main__":
    main()

# trasformare in funzione riutilizzabile
